...
module.exports = {
    create: function (options) {
        ...
        if (!options.password) {
            logger.warn(
                `No admin CLI password specified, make sure port ${options.port} is secured`
            );
        }
        ...
        net.createServer(function (socket) {
            ...
            const printNode = (socket, i, node) => {
                ...
                socket.write(
                    ...
                    } [${node.getTaskQueueCount()}/${node.getMaxParallelTasks()}] <engine: ${node.getEngineInfo()}> <API: ${node.getVersion()}>${
                ...
                if (loggedIn) {
                    if (command === "HELP") {
                        ...
                        if (subcommand === "ADD" && args.length >= 2) {
                            ...
                            const [number] = args;
                            reply(
                                await netutils.removeAndCleanupNode(
                                    nodes.nth(number),
                                    asrProvider.get()
                                )
                            );
                            ...
                            const node = await nodes.findBestAvailableNode(
                                numImages
                            );
                            if (node) {
                        ...
                        if (subcommand === "LIST") {
                            ...
                            if (number !== undefined && !node) invalid();
                                ...
                                jsonResponse(
                                    await netutils.findTasksByNode(node)
                                );
                        } else if (subcommand === "INFO" && args.length >= 1) {
                            const [taskId] = args;
                            const route = await routetable.lookup(taskId);
                            if (!route) invalid();
                                ...
                                const taskInfo = await route.node.taskInfo(
                                    taskId
                                );
                            ...
                            if (!route) invalid();
                                ...
                                const taskOutput = await route.node.taskOutput(
                                    taskId,
                                    line
                                );
                            ...
                            ["CANCEL", "REMOVE"].indexOf(subcommand) !== -1 &&
                            args.length >= 1
                        ) {
                            const [taskId] = args;
                            const route = await routetable.lookup(taskId);
                        ...
                        if (subcommand === "VIEWCMD") {
                            const [numImages] = args;
                            const cmd = await asr.debugCreateDockerMachineCmd(
                                numImages
                            );
                            socket.write(`${cmd}\r\n`);
                        } else {
