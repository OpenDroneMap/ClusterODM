        ...
        let domainChecks = [
            {
                ...
                validate: function (matches, value) {
                    ...
                    else if (matches[2] === "integer")
                        return Number.isInteger(value);
                    else if (matches[2] === "float")
        ...
        let checkDomain = function (domain, value) {
            if (Array.isArray(domain)) {
                ...
                if (domain.indexOf(value) === -1)
                    throw new Error(`Invalid value ${value} (not in enum)`);
            } else {
                let matches,
                    dc = domainChecks.find(
                        (dc) => (matches = domain.match(dc.regex))
                    );

                if (dc) {
                    if (!dc.validate(matches, value))
                        throw new Error(
                            `Invalid value ${value} (out of range)`
                        );
                } else {
                    throw new Error(
                        `Domain value cannot be handled: '${domain}' : '${value}'`
                    );
                }
            }
        };
