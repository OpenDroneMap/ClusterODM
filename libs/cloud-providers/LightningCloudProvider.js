...
module.exports = class LightningCloudProvider extends AbstractCloudProvider {
    ...
    async validate(token) {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                logger.warn(
                    `Cannot validate token ${token}, returned status ${response.status}`
                );
                return {
                    valid: false,
                };
    ...
    async approveNewTask(token, imagesCount) {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                logger.warn(
                    `Cannot approve task with ${token}:${imagesCount}:${resolution}, returned status ${response.status}`
                );
                return {
                    error: `Cannot approve task. Please contact support. Service responded with status ${response.status}`,
                };
            }
        } catch (e) {
            logger.warn(
                `Cannot approve task with ${token}:${imagesCount}:${resolution}: ${e.message}`
            );
            return {
                error: `Cannot approve task. Please contact support. ${e.message}`,
            };
        }
    }
    ...
    async taskFinished(token, taskInfo) {
        if (!token) {
            ...
            logger.error(
                `Cannot record transaction, token is missing: ${JSON.stringify(
                    taskInfo
                )}`
            );
            return;
            ...
            try {
                ...
                if (response.status === 200) {
                    ...
                    if (!error) {
                        ...
                        logger.error(
                            `/tasks/finished returned: ${error}, attempt (${i})`
                        );
                    }
                } else {
                    logger.error(
                        `Cannot call /tasks/finished for ${taskInfo.uuid}, ${token}, returned status ${response.status}, attempt (${i})`
                    );
                }
            } catch (e) {
                logger.error(
                    `Cannot call /tasks/finished for ${taskInfo.uuid}, ${token}: ${e.message}, attempt (${i})`
                );
            }
    ...
    handleAuthInfo(req, res) {
        ...
        res.end(
            JSON.stringify({
                message:
                    "Please enter your webodm.net credentials. If you don't have an account, register for free at https://webodm.net/register",
