...
module.exports = {
    initialize: async function (cloudProvider) {
        ...
        const getLimitedOptions = async (token, limits, node) => {
            ...
            const limitedOptions = odmOptions.optionsWithLimits(
                options,
                limits.options
            );
        ...
        const maxConcurrencyLimitReached = async (
            maxConcurrentTasks,
            token
        ) => {
                            ...
                            .then((taskInfo) => {
                                if (
                                    ...
                                    [
                                        statusCodes.QUEUED,
                                        statusCodes.RUNNING,
                                    ].indexOf(taskInfo.status.code) !== -1
                                )
                                    runningTasks++;
                                cb();
                            });
        ...
        const pathHandlers = {
            "/info": function (req, res, user) {
                ...
                json(res, {
                    ...
                    maxParallelTasks:
                        limits.maxConcurrentTasks !== undefined
                            ? limits.maxConcurrentTasks
                            : 99999999999,
                    engineVersion:
                        node !== undefined ? node.getInfo().engineVersion : "?",
            ...
            "/options": async function (req, res, user) {
                ...
                if (!node)
                    json(res, {
                        error: "Cannot compute /options, no nodes are online.",
                    });
                else {
                    const options = await getLimitedOptions(
                        token,
                        limits,
                        node
                    );
        ...
        const requestListener = async function (req, res) {
            ...
            try {
                ...
                if (req.method === "POST" && pathname === "/commit") {
                    ...
                    try {
                        ...
                        try {
                            ...
                            logger.error(
                                `cloudProvider.taskFinished: ${e.message}`
                            );
                        }
                ...
                // Validate user token
                const {valid, limits} = await cloudProvider.validate(
                    query.token
                );
                ...
                if (req.method === "POST" && pathname === "/task/new/init") {
                    ...
                    taskNew.formDataParser(req, async function (params) {
                        ...
                        if (
                            await maxConcurrencyLimitReached(
                                limits.maxConcurrentTasks,
                                query.token
                            )
                        ) {
                            ...
                            die(
                                `Reached maximum number of concurrent tasks: ${limits.maxConcurrentTasks}. Please wait until other tasks have finished, then restart the task.`
                            );
                            return;
                        }
                        ...
                        // Validate options
                        try {
                            odmOptions.filterOptions(
                                options,
                                await getLimitedOptions(
                                    query.token,
                                    limits,
                                    referenceNode
                                )
                        ...
                        if (floodMonitor.isFlooding(query.token)) {
                            die(
                                `Uuh, slow down! It seems like you are sending a lot of tasks. Check that your connection is not dropping, or wait ${floodMonitor.FORGIVE_TIME} minutes and try again.`
                            );
                            return;
                        }
                    ...
                    req.method === "POST" &&
                    pathname.indexOf("/task/new/upload") === 0
                ) {
                                    ...
                                    if (limits && limits.maxImages) {
                                        ...
                                        fs.readdir(
                                            ...
                                            (err, files) => {
                                                if (err) {
                                                    logger.warn(
                                                        `Failed to read files from ${saveFilesToDir}`
                                                    );
                                                    cb();
                                                } else if (
                                                    files.length - 1 >
                    ...
                    req.method === "POST" &&
                    pathname.indexOf("/task/new/commit") === 0
                ) {
                    const taskId = taskNew.getTaskIdFromPath(pathname);
                    if (taskId) {
                    ...
                    req.method === "POST" &&
                    ["/task/restart", "/task/cancel", "/task/remove"].indexOf(
                        pathname
                    ) !== -1
                ) {
                    ...
                    busboy.on("finish", async function () {
                        if (taskId) {
                            ...
                            if (node) {
                                ...
                                const taskTableEntry = await tasktable.lookup(
                                    taskId
                                );
                                if (taskTableEntry && taskTableEntry.taskInfo) {
                                    if (
                                        pathname === "/task/cancel" ||
                                        pathname === "/task/remove"
                                    ) {
                                        if (taskTableEntry.abort) {
                                            taskTableEntry.abort();
                                            taskTableEntry.abort = null;
                                            logger.info(
                                                `Task ${taskId} aborted via ${pathname}`
                                            );
                                        }
                                        ...
                                        if (pathname === "/task/cancel") {
                                            taskTableEntry.taskInfo.status.code =
                                    ...
                                    json(res, {
                                        error: `Invalid route for taskId ${taskId}, no nodes in routing table.`,
                                    });
                                }
                            }
                        } else {
                    ...
                    const taskIds = {};
                    const taskTableEntries = await tasktable.findByToken(
                        query.token
                    );
                    ...
                    const routeTableEntries = await routetable.findByToken(
                        query.token,
                        true
                    );
                    ...
                    if (matches && matches[1]) {
                        ...
                        if (
                            asrProvider.downloadsPath() &&
                            action.indexOf("download") === 0
                        ) {
                            const assetsMatch =
                                action.match(/^download\/(.+)$/);
                            if (assetsMatch && assetsMatch[1]) {
                                let assetPath = assetsMatch[1];

                                // Special case for orthophoto.tif
                                if (assetPath === "orthophoto.tif")
                                    assetPath =
                                        "odm_orthophoto/odm_orthophoto.tif";

                                const s3Url = url.parse(
                                    asrProvider.downloadsPath()
                                );
                            ...
                            if (taskTableEntry) {
                                ...
                                if (action === "info") {
                                    ...
                                    if (query.with_output !== undefined) {
                                        const line =
                                            parseInt(query.with_output) || 0;
                                        const output =
                                            taskTableEntry.output || [];
                                        response.output = output.slice(
                                            line,
                                            output.length
                                        );
                                    }
                                    ...
                                    json(
                                        res,
                                        output.slice(line, output.length)
                                    );
                                } else {
                                    json(res, {
                                        error: `Invalid route for taskId ${taskId}:${action}, no valid route possible.`,
                                    });
                                }
                            } else {
                                json(res, {
                                    error: `Invalid route for taskId ${taskId}:${action}, no task table entry.`,
                                });
                            }
                        }
                    } else {
