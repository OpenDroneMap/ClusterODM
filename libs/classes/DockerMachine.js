            ...
            childProcess
                .on("exit", (code, signal) => {
                    if (code === 0) {
                        ...
                        reject(
                            new Error(
                                "Docker-machine does not seem to work. Please make sure docker-machine is working."
                            )
                        );
                })
                .on("error", () =>
                    reject(
                        new Error(
                            "Docker-machine not found in PATH. Please install docker-machine if you want to use the autoscaler."
                        )
                    )
                );
            childProcess;
    ...
    Class: class DockerMachine {
        ...
        async run(args) {
            ...
            return new Promise((resolve, reject) => {
                ...
                runningProcesses[this.machineName] =
                    runningProcesses[this.machineName] || [];
                runningProcesses[this.machineName].push(childProcess);

                const cleanup = () => {
                    runningProcesses[this.machineName] = runningProcesses[
                        this.machineName
                    ].filter((p) => p !== childProcess);
                    if (runningProcesses[this.machineName].length === 0)
                        delete runningProcesses[this.machineName];
                };
                const output = [];
                ...
                childProcess
                    .on("exit", (code, _) => {
                        ...
                        if (code === 0) resolve(output.join("\n"));
                            ...
                            reject(
                                new Error(
                                    `docker-machine exited with code ${code}`
                                )
                            );
                    })
                    .on("error", () => {
        ...
        async getIP() {
            ...
            if (info && info.Driver && info.Driver.IPAddress) {
                ...
                throw new Error(
                    `Cannot get IP for machine: ${this.machineName}`
                );
            }
        }
