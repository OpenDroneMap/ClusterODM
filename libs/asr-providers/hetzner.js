...
module.exports = class HetznerAsrProvider extends AbstractASRProvider {
    ...
    async initialize() {
        ...
        if (sshKeyPath) {
            ...
            const exists = await new Promise((resolve) =>
                fs.exists(this.getConfig("sshKey.path"), resolve)
            );
            if (!exists)
                throw new Error(
                    "Invalid config key sshKey.path: file does not exist"
                );
        }
    ...
    async getSSHKeyId() {
        ...
        if (!fingerprint)
            throw new Error(
                "Need to specify SSH Key fingerprint in configuration."
            );
        if (this.fpCache[fingerprint]) return this.fpCache[fingerprint];

        // We need to fetch the ssh key ID
        const response = await axios.get(
            `https://api.hetzner.cloud/v1/ssh_keys?page=1&per_page=9999999&fingerprint=${encodeURIComponent(
                fingerprint
            )}`,
            {
                timeout: 10000,
        ...
        if (response.status === 200) {
            ...
            if (key && key.id) {
                ...
                throw new Error(
                    `SSH Key with fingerprint ${fingerprint} not found.`
                );
            }
        } else {
    ...
    async getSnapshotId() {
        ...
        const response = await axios.get(
            "https://api.hetzner.cloud/v1/images?type=snapshot&description=nodeodm-image&page=1&per_page=9999999",
            {
                timeout: 10000,
                headers: {
                    Authorization: `Bearer ${this.getConfig("apiToken")}`,
