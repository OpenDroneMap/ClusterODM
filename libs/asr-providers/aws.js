...
module.exports = class AWSAsrProvider extends AbstractASRProvider {
    ...
    async setupMachine(req, token, dm, nodeToken) {
        ...
        if (swapToMemRatio) {
            ...
            if (!isNaN(memory)) {
                await dm.ssh(
                    `bash -c "sudo fallocate -l ${Math.ceil(
                        memory * swapToMemRatio * 1024
                    )} /swapfile && sudo chmod 600 /swapfile && sudo mkswap /swapfile && sudo swapon /swapfile && free -h"`
                );
            } else {
        ...
        await dm.ssh(
            [
                `sudo docker run -d -p 3000:3000 ${dockerImage} -q 1`,
