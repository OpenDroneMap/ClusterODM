    ...
    async getImageInfo() {
        ...
        if (this.getConfig("snapshot")) {
            ...
            const response = await axios.get(
                "https://api.digitalocean.com/v2/images?page=1&per_page=9999999&private=true",
                {
                    timeout: 10000,
                    headers: {
                        Authorization: `Bearer ${this.getConfig(
                            "accessToken"
                        )}`,
                    },
                }
            ...
            if (response.status === 200) {
                ...
                if (img && img.id && img.regions) {
                    ...
                    if (img.regions.indexOf(imageRegion) === -1) {
                        if (img.regions.length > 0) {
                            ...
                            logger.warn(
                                `The snapshot ${imageName} is not available in the ${imageRegion}, switching to ${newRegion}`
                            );
                            imageRegion = newRegion;
                        } else {
                            // Can this ever happen?
                            throw new Error(
                                `Snapshot found, but not available in any region.`
                            );
                        }
                    }
                ...
                throw new Error(
                    `Cannot contact DigitalOcean API: ${response.status}`
                );
            }
        }
